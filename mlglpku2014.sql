...
--
-- Host: 127.0.0.1
-- Generation Time: Nov 09, 2014 at 08:15 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_keyword`(IN inKeywordTerm VARCHAR(100))
BEGIN
 INSERT INTO keyword (keyword_term) VALUES (inKeywordTerm);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `add_keyword_to_module`(IN inModuleID INT, IN inKeywordID INT)
BEGIN
 INSERT INTO module_keyword (module_id, keyword_id) VALUES (inModuleID, inKeywordID);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `add_module`(IN inModuleTitle TEXT, IN inModuleLeader TEXT, IN inModuleDescription TEXT, IN inImage TEXT, IN inCategory TEXT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `count_keyword_category_result`(IN inKeyword TEXT, IN inCategory TEXT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `count_modules_in_collection`()
BEGIN
  SELECT COUNT(*) AS modules_in_collection_count
  FROM   module;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `count_modules_in_collection_by_category`(IN inCategory TEXT)
BEGIN
  SELECT COUNT(*) AS modules_in_collection_count
  FROM   module
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `count_modules_in_collection_by_keyword`(IN inKeyword TEXT)
BEGIN
  SELECT COUNT(*) AS modules_in_collection_count
  FROM   module, module_keyword, keyword
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `count_search_category_result`(IN inSearchString TEXT, IN inCategory TEXT)
BEGIN
    PREPARE statement FROM
      "SELECT   count(*)
       FROM     module
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `count_search_keyword_result`(IN inSearchString TEXT, IN inKeyword TEXT)
BEGIN
    PREPARE statement FROM
      "SELECT   count(*)
       FROM     module, module_keyword, keyword
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `count_search_result`(IN inSearchString VARCHAR(50))
BEGIN
    PREPARE statement FROM
      "SELECT   count(*)
       FROM     module
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_category_list`()
BEGIN
	SELECT module_category, count(*) as count FROM module group by module_category order by count desc, module_category;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_category_list_search`(IN inSearchString TEXT)
BEGIN
    PREPARE statement FROM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_keyword_category_list`(IN inKeyword TEXT)
BEGIN
    PREPARE statement FROM
      "SELECT   module_category, count(*) as count
       FROM     module, module_keyword, keyword
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_modules_by_category`(IN inCategory TEXT, IN inModulesPerPage INT, IN inStartItem INT)
BEGIN
    SET @p1 = inCategory;
	SET @p2 = inStartItem;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_modules_by_keyword`(IN inKeyword TEXT, IN inModulesPerPage INT, IN inStartItem INT)
BEGIN
    SET @p1 = inKeyword;
	SET @p2 = inStartItem;
	SET @p3 = inModulesPerPage;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_modules_by_keyword_category`(IN inKeyword TEXT, IN inCategory TEXT, IN inModulesPerPage INT, IN inStartItem INT)
BEGIN
    SET @p1 = inKeyword;
    SET @p2 = inCategory;	
	SET @p3 = inStartItem;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_modules_latest`()
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_modules_list`()
BEGIN
	SELECT module_id, module_title, module_leader, module_description, module_image, module_category FROM module ORDER BY module_id desc limit 12;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_modules_on_collection`(IN inModulesPerPage INT, IN inStartItem INT)
BEGIN
	PREPARE statement FROM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_module_details`(IN inModuleId INT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_module_keywords`(IN inModuleId INT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_module_leaders_list`()
BEGIN
	SELECT module_leader, count(*) as modules FROM module Group By module_leader Order by modules desc, module_leader;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_module_leaders_modules`(IN inModuleLeader TEXT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_module_leader_modules`(IN inModuleLeader TEXT)
BEGIN
	SET @pattern = CONCAT('%',inModuleLeader,'%');
	PREPARE statement FROM
      "SELECT   module_id, module_title,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_module_moduleleader`(IN inModuleId INT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_random_module`()
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_random_modules`()
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `search`(IN inSearchString VARCHAR(50), IN inModulesPerPage INT, IN inStartItem INT)
BEGIN
    PREPARE statement FROM
      "SELECT   module_id, module_title,
                module_leader, module_description, module_image, module_category
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `search_category`(IN inSearchString TEXT, IN inCategory TEXT, IN inModulesPerPage INT, IN inStartItem INT)
BEGIN
    PREPARE statement FROM
      "SELECT   module_id, module_title,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `search_keyword`(IN inSearchString TEXT, IN inKeyword TEXT, IN inModulesPerPage INT, IN inStartItem INT)
BEGIN
    PREPARE statement FROM
      "SELECT   module.module_id, module_title,
                module_leader, module_description, module_image
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_module`(IN inModuleId INT, IN inModuleTitle TEXT, IN inModuleLeader TEXT, IN inModuleDescription TEXT, IN inmodule_image TEXT, IN inCategory TEXT)
BEGIN
	UPDATE module
...
(37, 'Web Programming 2', 'Edmond Hobeika', 'Web programming 2 will be handling all of the PHP & Java Script language during the length of the course and will teach how to run a code through a database and display the results out on a HTML CSS web page.', 'Question_mark.PNG', 'Computing'),
(38, 'Software Process and Practices', 'Edmond Hobeika', 'Software Process and Practices is a module that teaches how to develop testing units for Java application.', 'Question_mark.PNG', 'Computing'),
(39, 'Database Systems Development 1', 'Edmond Hobeika', 'This module practices the use of databases of which data is stored and is retrieved to display on webpages by many different languages.', 'Question_mark.PNG', 'Computing'),
...
(145, 'Blade', 'Eric Brooks', 'Blade is a Superhero created by Marvel, he is a Half Human Half Vampire, who hunts Vampires.', 'ls_Blade.jpg', 'Film and TV');
...
--
-- Indexes for table `module_keyword`
--
...
--
ALTER TABLE `module`
MODIFY `module_id` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=146;
--
